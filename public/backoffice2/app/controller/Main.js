   ...
   onsearchclick: function () {       
        ...
        if (txtobj.getValue() != "") {
            ...
            for (var i = 0; i < Dataarry.length; i++) {
                ...
                if (Dataarry[i]["customer_name"].toLowerCase().search(searchval.toLowerCase()) != -1) {
                    flag = false;
                    finaldata_Ary.push(Dataarry[i]);
                }
            }
    ...
    setOrderList: function (records) { 
        ...
        var ref = this;
        var str = '';
        str += '<div class="table-responsive" id="grid"><table class="table table-bordered" id=""><thead><tr class="headerrow"><th onclick="orderidfilter()">Order No</th><th>Delivery Slot</th><th>Name</th><th>Mobile</th><th>Payment</th><th>Progress</th><th>Meal Type</th><th onclick="orderidfilter()">Order Time</th><th></th><th>Area</th><th>Sub-Area</th></tr></thead><tbody>';
        if(HolaChef.app.Orderidfilter != 0)
...
for (var i = 0; i < records.length; i++) {
            ...
            if(date != null)
            {
            var t1= timeNow.getHours();
            var t1= t1+':';
            var t2= timeNow.getMinutes();
            }
            else{
              t1='null';
              t2='';
            ...
            str += '<td><a href="#" style="text-decoration:none;color:black;" onclick="OrderdetailPopup(\'' + String(id) + '\',\'orderno\');">' + (records[i]["order_items"][0] == undefined ? "" : records[i]["order_items"][0]["meal_type"]) + '</a></td><td><a href="#" style="text-decoration:none;color:black;" onclick="OrderdetailPopup(\'' + String(id) + '\',\'orderno\');">' + t1 + '' + t2 +'</td><td style="padding-left: 5px;padding-right: 5px;"><a href="#" style="text-decoration:none;color:black;" onclick="OrderdetailPopup2(\'' + String(id) + '\',\'orderno\');"><img style="width:40px;" src="resources/images/print.png"></td><td><a href="#" style="text-decoration:none;color:black;" onclick="OrderdetailPopup(\'' + String(id) + '\',\'orderno\');">' + records[i]["address"].area + '</td><td><a href="#" style="text-decoration:none;color:black;" onclick="OrderdetailPopup(\'' + String(id) + '\',\'orderno\');">' + records[i]["address"].sub_area + '</td></a>';
            str += '</a>';
...
function viewordetail(id,from)
{
    ...
    Ext.data.JsonP.request({
        ...
        success: function (records) {
            ...
            html+='</td></tr><tr><td><table cellpadding="0" cellspacing="0" width="100%"><tr><td style="border:1px solid black;padding:5px;width:33%"><table cellpadding="0" cellspacing="0"><tr><td>' + records["customer_name"] + '</td></tr><tr><td>' + records["customer_contact_number"] + '</td></tr>';
            ...
            html += '</table></div></td><td style="border:1px solid black;padding:5px;width:33%"><table cellpadding="0" cellspacing="0"><tr><td >Order progress : </td><td style="width:60%"> ' + records["order_progress"] + '</td></tr><tr><td>Payment : </td><td>' + records["payment_mode"] + '</td></tr><tr><td>Delivery Time : </td><td>' + records["deliverySlot"] + '</td></tr><tr><td>Runner Name :</td><td>' + records["runner"].name + '</td></tr>';
            ...
            html += '</table><td style="border:1px solid black;padding:5px;width:33%"><table cellpadding="0" cellspacing="0"><tr><td>Area</td><td><select id="areadropdown" onchange="changearea()"><option value="0">Select Area</option><option value="powai">powai</option><option value="jogeshwari">jogeshwari</option></select></td></tr><tr><td>SubArea</td><td><select id="subareadropdown"  style="margin-top:5px;" onchange="changesubarea(\'' + String(records["id"]) + '\')"></select></td></tr></table></td></td></tr></table></td></tr><tr><td><div style="float: right; width: 30%"><select id="ddlaction"><option value="0">Select Action</option><option value="Canceled">Canceled</option><option value="Damaged">Damaged</option>';
...
function viewordetail2(id,from)
{
...
function changearea()
{
      if (document.getElementById("areadropdown").value == "powai"){
          var powai = new Array("powaiArea1","powaiArea2");
          document.getElementById("subareadropdown").innerHTML ='<option value="Select SubArea">Select SubArea</option>';
          for (var i=0; i < powai.length;i++)
          {
            
            AddItem(powai[i],powai[i]);
          }
      
      }
      else if(document.getElementById("areadropdown").value == "jogeshwari")
      {
         var jogeshwari = new Array("jogeshwariArea1","jogeshwariArea2");
          document.getElementById("subareadropdown").innerHTML ='<option value="Select SubArea">Select SubArea</option>';
          for (var i=0; i < jogeshwari.length;i++)
          {
            
            AddItem(jogeshwari[i],jogeshwari[i]);
          }
      }
}
 function AddItem(Text,Value)

    {
      ...
      // Add an Option object to Drop Down/List Box
        ...
        document.getElementById("subareadropdown").options.add(opt);
...
function changesubarea(id)
{
  ...
  var area=document.getElementById("areadropdown").value;
  var subarea=document.getElementById("subareadropdown").value;
  var urlstr = '/api_v2/' + 'orders/' + id + '/update_area_and_sub_area.json?area=' + area +'&sub_area=' + subarea +'&order_id='+id;
  ...
  $.ajax({
        ...
        dataType: "jsonp",
        success: function (response) {//On Successfull service call
            Ext.Msg.alert('Alert', 'Area subArea updated sussesfully.');
        },
        failure: function (response) {
